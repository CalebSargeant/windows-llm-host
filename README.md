...
.\scripts\detect-model.ps1 -Json            # machine-readable, for scripts / the GUI
```

VRAM governs speed (a model that fits in VRAM runs on the GPU); system RAM governs how big you can go (larger models offload to CPU/RAM). The detector ranks candidates from `qwen3:1.7b` up to the 70B-class models and reports fast / balanced / coding / max picks for your machine.
...
Go heavier only after benchmarking: `qwen3:14b-q4_K_M`, `gpt-oss:20b`, the 30B Qwen3 models, `qwen2.5-coder:32b`, and the 70B-class `llama3.3:70b` (`max` profile). The `extreme` profile (`qwen2.5:72b`, `gpt-oss:120b`) is opt-in and can exceed 64 GB RAM. These are included for maximum capability, not comfort.
